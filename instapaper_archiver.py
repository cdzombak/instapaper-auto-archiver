...
class Rules(object):
    ...
    def add_rules(self, rules_dict):
        ...
        if "domain_specific" not in rules_dict:
            raise Rules.SpecException(
                "Rules file must contain a domain_specific rules list."
            )
        for rule in rules_dict["domain_specific"]:
            if "domain" not in rule or "max_age" not in rule:
                raise Rules.SpecException(
                    "Domain rule {} must include a domain and max_age.".format(
                        json.dumps(rule)
                    )
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Archive old, unread Instapaper bookmarks."
    )
    parser.add_argument(
        "action", nargs="?", default="run", choices={"run", "list-domains"}
    )
    parser.add_argument(
        "--dry-run",
        type=str2bool,
        ...
        "False to archive old unread bookmarks. Default: True.",
    )
    parser.add_argument(
        "--entries-limit",
        type=int,
        default=250,
        help="Max number of bookmarks to fetch via the Instapaper API. Default: 250. Max: 500.",
    )
    parser.add_argument(
        "--max-age",
        type=int,
        default=90,
        help="Bookmarks older than this many days will be marked as read."
        "Ignored if using --rules-file. Default: 90.",
    )
    parser.add_argument(
        "--only-domain",
        default=None,
        help="Operate on only bookmarks from the given domain. Default: none.",
    )
    parser.add_argument(
        "--rules-file",
        default=None,
    ...
    if (
        not instapaper_api_id
        or not instapaper_api_secret
        or not instapaper_login
        or not instapaper_pass
    ):
        eprint(
            "Instapaper API key & credentials must be set using environment variables."
        )
    ...
    if args.action == "run":
        ...
        if args.rules_file:
            ...
            try:
                ...
                sys.exit(1)
        run_archive(
            ip_api, entries_limit=args.entries_limit, rules=rules, dry_run=args.dry_run
        )
        sys.exit(0)
